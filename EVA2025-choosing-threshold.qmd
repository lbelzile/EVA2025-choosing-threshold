...
Consider selecting a threshold among candidates $u_1 < \cdots < u_k.$
...
@smith_1987 show that a better approximation is obtained by letting the shape vary with $u$, with $\xi_t = r'(u),$ where $$r(t) = \{1-F(t)\}/f(t)$$ is the reciprocal hazard function.
...
- Problems: sample overlap leads to dependence between estimates (pointwise confidence intervals).
...
Some model are inspired by penultimate approximations (GPD-like, but with varying shape).
...
- Many numerical methods in **R** packages `mev` and `tea`.

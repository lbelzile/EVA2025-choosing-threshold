...
\begin{align*}
\Lambda[(t',t)\times[u,\infty)] = (t_2-t_1)\{1+\xi(u-\eta)/\tau\}^{-1/\xi}_+, \qquad \eta \in \mathbb{R}, \tau > 0
\end{align*}


The vertical coordinates of ${\mathcal P}$ on  ${\mathcal C}_{-\infty}$ can be generated as 
\begin{align*}
\eta + \frac{\tau}{\xi}\Bigg\{\Bigg(\sum_{j=1}^r E_j\Bigg)^{-\xi} - 1\Bigg\}, \quad r=1,2,\ldots,
\end{align*}
...
\begin{equation}
\label{eq:Renyi}
\prod_{j=2}^n f(x_{(j)} \mid x_{(j-1)}) f(x_{(1)})= \prod_{j=1}^n \frac{1}{\sigma_j}\left\{1 + \frac{\xi}{n+1-j}\frac{(y_{(j)}-y_{(j-1)})}{ \sigma_j}\right\}^{-(n+1-j)/\xi-1}_+,
\end{equation}
where $\sigma_j = (\sigma+\xi y_{j-1})/(n+1-j)$ and $y_{(0)}=0$.
...
Arguments of @smith_1987 show that a better approximation is obtained by letting the shape vary with $u$, with 
...
- Mean residual life plots [@Davison.Smith:1990]
...
@Northrop.Coleman:2014: piecewise generalized Pareto model, score tests for equality.
...
## Extended generalized Pareto models {.smaller}
...
- @Papastathopoulos.Tawn:2013 (their models imply the density at the origin is zero)
- @Gamet.Jalbert:2022 (other models, but non-regular asymptotics).
...
- Must choose the threshold sequence $u_1 \leq \cdots u_k$ carefully.
...
@Northrop.Attalides.Jonathan:2017 propose a related Bayesian method based on leave-one-out cross validation with a binomial-generalized Pareto (BGP) model and a single validation threshold $v > u_k$ above which we assess the model performance.
...
## Simulation study: comparison of methods {.smaller}
...
The paper also compares 18 different methods using Hill-type  estimator for heavy-tailed data.

- Careful: many numerical implementations don't specify a minimum sample size!
- Primer: methods based on minimization of the asymptotic mean squared error of the Hill estimator [@Caeiro.Gomes:2016], or the double nonparametric bootstrap scheme @Gomes.Figueiredo.Neves:2012, and random block maxima estimator of @Wager:2014 with empirical risk minimization perform best

## Don't use the following {.smaller}

- Methods based on minimization of the asymptotic mean squared error can break down catastrophically for particular datasets.
...
- @Drees.Kaufmann:1998 bias-reduction method leads to small number of order statistics, with very large width of the confidence intervals (unwanted variability).
